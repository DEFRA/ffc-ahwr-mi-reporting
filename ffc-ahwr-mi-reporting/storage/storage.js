...
const {
  ...
  buildColumns,
} = require("../mi-report-v3/transformJsonToCsvV3");
...
const EVENT_TYPES_NOT_NEEDED_BY_REPORTING_TEAM = new Set([
  "tokens-nonce",
  "tokens-state",
  "pkcecodes-verifier",
]);
...
const processEntitiesByTimestampPaged = async (fileName, context) => {
  ...
  if (!exists) {
    ...
    await appendBlobClient.appendBlock(
      headerContent,
      Buffer.byteLength(headerContent),
    );
  ...
  for await (const eventsPage of eventsIterator) {
    ...
    if (rowContent) {
      await appendBlobClient.appendBlock(
        rowContent,
        Buffer.byteLength(rowContent),
      );
      context.log.info(
        `Page ${pageCount} and ${eventItemCount} event items written to append blob`,
      );
    } else {
      context.log.info(
        `Page ${pageCount} was not written to csv, no rowContent produced for eventsPage`,
      );
    }
  }
};
...
const streamBlobToFile = async (fileName) => {
  ...
  return {
    fileContentStream: downloadBlockBlobResponse.readableStreamBody,
    contentLength,
  };
};
