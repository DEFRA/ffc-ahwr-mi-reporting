...
const getDriveId = async (siteId, accessToken, context) => {
  ...
  if (drive === undefined) {
    throw new Error(
      `No drive found: ${JSON.stringify({ name: config.sharePoint.documentLibrary })}`,
    );
  }
  return drive.id;
};
...
const uploadBlobToSharePoint = async (
  ...
  context.log.info(
    `Uploading file: fileName: ${fileName}, pathToFile: ${pathToFile}`,
  );
  ...
  try {
    for await (const chunk of readableStream) {
      ...
      while (buffer.length >= chunkSize) {
        ...
        context.log.info(
          `Uploading bytes ${start}-${end}/${totalSize} to ${uploadUrl}`,
        );
    ...
    if (buffer.length > 0) {
      ...
      context.log.info(
        `Uploading final bytes ${start}-${end}/${totalSize} to ${uploadUrl}`,
      );
...
const createUploadSession = async (
  ...
  try {
    ...
    context.log.info(
      `Upload session created: ${JSON.stringify(response.payload)}`,
    );
    return JSON.parse(response.payload.toString("utf8"));
  } catch (error) {
    context.log.error(`Error creating upload session: ${error.message}`);
    if (error.data) {
      context.log.error(`Graph error: ${JSON.stringify(error.data, null, 2)}`);
    }
    throw error;
  }
};
