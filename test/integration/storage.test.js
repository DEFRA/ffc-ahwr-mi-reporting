...
jest.mock("@azure/storage-blob", () => {
  ...
  return {
    ...actual,
    BlobServiceClient: jest
      .fn()
      .mockImplementation(
        () =>
          new actual.BlobServiceClient(
            blobUrl,
            new actual.StorageSharedKeyCredential(account, key),
          ),
      ),
  };
});
...
const {
  ...
  key,
  blobUrl,
} = require("./azurite-config");
const { TableClient } = jest.requireActual("@azure/data-tables");
const { BlobServiceClient, StorageSharedKeyCredential } = jest.requireActual(
  "@azure/storage-blob",
);

const seedTableClient = TableClient.fromConnectionString(
  connectionString,
  tableName,
  { allowInsecureConnection: true },
);
const azuriteBlobService = new BlobServiceClient(
  blobUrl,
...
async function readBlob(fileName) {
  const blobClient = azuriteBlobService
...
describe("storage pipeline (integration)", () => {
  ...
  test("event data appears in the correct columns", async () => {
    ...
    const orgRow = rows.find(
      (row) => row.eventType === "farmerApplyData-organisation",
    );
  ...
  test("status events are written with correct statusId and statusName", async () => {
    ...
    const statusRow = rows.find(
      (r) => r.eventType === "application:status-updated:1",
    );
  ...
  test("CSV header contains poultry columns when POULTRY_RELEASE_DATE is set to a past date", async () => {
    featureToggleConfig.poultryReleaseDate = new Date(
      "2020-01-01",
    ).toISOString();
  ...
  describe("when a poultry event is seeded", () => {
    ...
    afterAll(async () => {
      await seedTableClient.deleteEntity(
        VALID_POULTRY_EVENT.partitionKey,
        VALID_POULTRY_EVENT.rowKey,
      );
    });
    ...
    test("poultry event appears as a data row with the correct value in the schemeType column", async () => {
      ...
      const { rows } = parseCsv(await readBlob(fileName));
      const poultryRow = rows.find(
        (row) => row.eventType === "scheme-schemeType",
      );
