...
describe("formatDate(date)", () => {
  test.each([
    ...
    expect(formatDate(testCase.given.date, testCase.when.format)).toEqual(
      testCase.expect.formattedDate,
    );
  });

  test("formatDate called with custom desired format outputs as expected", () => {
    const result = formatDate(
      new Date("2023-02-23T10:20:18.352Z"),
      moment.ISO_8601,
      "DD-MMM-YYYY",
    );
...
describe("getVisitDateFromPossibleSources", () => {
  test("returns visitDateValue when present", () => {
    const result = getVisitDateFromPossibleSources(
      "2024-01-01",
      undefined,
      undefined,
    );
...
describe("getVetNameFromPossibleSources", () => {
  test("returns vetNameValue when present", () => {
    const result = getVetNameFromPossibleSources(
      "some vet",
      undefined,
      undefined,
    );
...
describe("getVetRcvsFromPossibleSources", () => {
  test("returns vetRcvsValue when present", () => {
    const result = getVetRcvsFromPossibleSources(
      "1234567",
      "7654321",
      undefined,
      undefined,
...
describe("getTestResultFromPossibleSources", () => {
  test("returns vetNameValue when present", () => {
    const result = getTestResultFromPossibleSources(
      "positive",
      undefined,
      undefined,
    );
...
describe("getDateOfTestingFromPossibleSources", () => {
  test("returns visitDateValue when present", () => {
    const result = getDateOfTestingFromPossibleSources(
      "2025-08-22T00:00:00.000Z",
      undefined,
      undefined,
    );

    expect(result).toEqual("2025-08-22T00:00:00.000Z");
  });
  test("returns updatedValue when updatedProperty is dateOfTesting", () => {
    const result = getDateOfTestingFromPossibleSources(
      undefined,
      "dateOfTesting",
    ...
    expect(result).toEqual("2025-08-20T00:00:00.000Z");
  });
  test("returns empty string when other updated property", () => {
    const result = getDateOfTestingFromPossibleSources(
      undefined,
...
describe("parseData and parsePayload", () => {
  ...
  const events = [
    ...
    },
    {
      ...
      Payload:
        '{"type":"claim-organisation","message":"Session set for claim and organisation.","data":{"reference":"TEMP-1234-ABCD","applicationReference":"IAHW-1234-EFGH","organisation":{"sbi":"123456","farmerName":"Farmer Brown","name":"Brown Cow Farm","email":"brown@test.com.test","orgEmail":"brownorg@test.com.test","address":"Yorkshire Moors,AB1 1AB,United Kingdom","crn":"0123456789","frn":"9876543210"}},"raisedBy":"brown@test.com.test","raisedOn":"2024-03-05T15:57:39.590Z"}',
    },
    {
      partitionKey: "123456",
      SessionId: "789123456",
      ...
      Payload:
        '{"type":"farmerApplyData-declaration","message":"Session set for farmerApplyData and declaration.","data":{"reference":"TEMP-1234-ABCD","declaration":true},"raisedBy":"brown@test.com.test","raisedOn":"2024-01-04T21:27:23.530Z"}',
    },
  ];
  ...
  test("parsePayload returns latest event from events payload from eventType supplied", () => {
    ...
    expect(result.message).toEqual(
      "Session set for farmerApplyData and organisation.",
    );
  });
});
...
describe("parseSheepTestResults", () => {
  ...
  test("returns formatted string from sheepTestResults", () => {
    ...
    const result = parseSheepTestResults(
      sheepTestResults,
      undefined,
      undefined,
    );
    ...
    expect(result).toEqual(
      "Bluetongue  result positive  Scrapie  result negative",
    );
  });

  test("returns formatted string from updatedProperty", () => {
    const sheepTestResults = [
      { result: "clinicalSymptomsPresent", diseaseType: "liverFluke" },
    ];
    const result = parseSheepTestResults(
      sheepTestResults,
      undefined,
