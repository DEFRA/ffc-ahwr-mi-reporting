describe("config", () => {
  ...
  const FEATURE_TOGGLE_PATH =
    "../../../ffc-ahwr-mi-reporting/feature-toggle/config";
  const SHAREPOINT_CONFIG_PATH =
    "../../../ffc-ahwr-mi-reporting/sharepoint/config";
  ...
  test("uses STORAGE_ACCOUNT_NAME when it is set", () => {
    jest.replaceProperty(process, "env", {
      ...baseEnv,
      STORAGE_ACCOUNT_NAME: "account-name",
    });
  ...
  test("throws when STORAGE_ACCOUNT_NAME is not set", () => {
    jest.replaceProperty(process, "env", {
      ...baseEnv,
